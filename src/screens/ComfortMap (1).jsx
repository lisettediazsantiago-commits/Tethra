...
// Every key that belongs to the CURRENT taxonomy. Anything saved outside this set
// is an orphan left behind when items were retired (e.g. the old physical-touch
// category that moved to the dedicated Intimacy section) and gets pruned on load.
const VALID_KEYS = new Set(
  COMFORT_CATEGORIES.flatMap((c) => c.items.map((it) => keyOf(c.key, it)))
);
...
export default function ComfortMap() {
  ...
  // Load once — and self-heal any answers left over from retired taxonomy items.
  useEffect(() => {
    if (!user) return;
    getDoc(doc(db, "comfortMaps", user.uid)).then(async (snap) => {
      if (snap.exists()) {
        ...
        const orphaned = Object.keys(stored).length - Object.keys(cleaned).length;
        setMap(cleaned);
        loaded.current = true;
        // If orphans existed, rewrite the whole doc so they're gone for good.
        // (A merge write can't delete absent keys — this must be a full replace.
        //  The doc only ever holds userId/items/updatedAt, so this is lossless.)
        if (orphaned > 0) {
          try {
            await setDoc(doc(db, "comfortMaps", user.uid), {
              userId: user.uid, items: cleaned, updatedAt: serverTimestamp(),
            });
          } catch {
            // Non-fatal: the on-screen view is already correct either way.
          }
        }
      } else {
        loaded.current = true;
