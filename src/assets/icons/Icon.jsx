...
const _customUrls = import.meta.glob("../assets/icons/*.svg", { eager: true, as: "url" });
const CUSTOM = {};
...
export default function Icon({ name, size = 22, color, bare = false, title, className, style }) {
  // Custom multi-color icons render on their tinted disc. In `bare` mode (the
  // bottom nav) we intentionally fall through to the line-art version instead,
  // so nav icons stay single-color and inherit the active/inactive tab color.
  const customSrc = bare ? undefined : CUSTOM[name];
  if (customSrc) {
